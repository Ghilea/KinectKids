...
The same `GastLivingFog` hierarchy is also created by `CorridorRunnerDirector` behind Greve. In the corridor `RearMass` fills the space behind him with an opaque, breathing black silhouette, while `EdgeMass` and `FloorMass` spread along the walls and floor. `TendrilSpawner` manages 7–10 independently timed `ProceduralTendril` tubes; each tube has 13 control points, Catmull–Rom interpolation, growth, pulse and retraction. `Wisps/Smoke` adds continuous mesh ribbons above the body. `GastFogController` exposes aggression from 0 to 1. The corridor also has a separate low blue fog bank around and in front of the player.
