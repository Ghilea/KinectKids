...
namespace KinectKids.Games.GreveGast.LivingFog
{
    public sealed class FloorMass : MonoBehaviour
    {
        ...
        public void Initialize(Material material, float wallWidth, bool corridorMode = false)
        {
            ...
            // The solid black floor covers blue wisps at the darkness front.
            // Its feathered near edge still reveals the player's floor fog.
            renderer.sortingOrder = corridorMode ? 26 : 20;
            Rebuild(0f);
        }
        ...
        private void Rebuild(float time)
        {
            ...
            for (int column = 0; column <= Columns; column++)
            {
                ...
                // In the corridor, overlap RearMass at z=2.3. Starting the
                // floor sheet in front of it left a bright uncovered strip
                // between Gast and the player.
                float z = corridor
                    ? 2.8f - (reach * reachMultiplier + 3.0f) * v
                ...
                if (corridor)
                {
                    ...
                    float distantCenter = 1f - LivingMassMesh.Smooth(0.18f, 0.50f, v);
